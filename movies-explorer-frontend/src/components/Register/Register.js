      ...
      <section className="auth__container">
        ...
        <form onSubmit={handleSubmit} className="auth__form">
          ...
          <div className="auth__input-container">
            <input className={`auth__input ${(name.minMaxLength && name.isDirty) ? `auth__input_type_error` : ``}`} required id="name" name="name" type="text" placeholder="Имя" minLength="2" maxLength="30" value={name.value} onChange={name.handleChange} onBlur={name.handleDirty}/>
          ...
          <button type="submit" className={`auth__button button-hovered ${handleButtonDisabled() ? `` : `auth__button_disable`}`} disabled={handleButtonDisabled() ? `` : true}>Зарегистрироваться</button>
        </form>
